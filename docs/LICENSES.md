...
### 6a. Pre-rendered speech — Kokoro-82M, Apache-2.0, build-time only

`build_files/66-prerender-speech.sh` synthesises the shell's own UI strings
into ~314 Ogg/Opus clips at `/usr/share/kidnix/speech/en_GB/` (~4.3 MB) and
ships **only the audio**. `docs/spikes/tts-kokoro.md` §6 rejected Kokoro as a
runtime engine (+460 MB of image, 415 MB resident, a hover label past the
700 ms threshold on a T480) and §7.1 accepted it as an asset; this is §7.1.

| Artefact | Version | Licence | Source | In the image? |
|---|---|---|---|---|
| **Kokoro-82M v1.0 weights** (`model.onnx`, 325,532,232 B, sha256 `8fbea51e…21a34cb`) | v1.0 ONNX export | **Apache-2.0** — HF front-matter on both repos | <https://huggingface.co/hexgrad/Kokoro-82M> · export <https://huggingface.co/onnx-community/Kokoro-82M-v1.0-ONNX> | **no** — fetched into a `mktemp -d`, used, deleted before the layer committed |
| **Voice `bf_emma`** (522,240 B, sha256 `669fe064…e4a63b73`) | same repo | **Apache-2.0**, no attribution owed | same | **no** |
| `config.json` (the 114-entry phoneme vocabulary, sha256 `5abb01e2…9b43c17f`) | hexgrad | Apache-2.0 | <https://huggingface.co/hexgrad/Kokoro-82M> | **no** |
| `onnxruntime` 1.29.0, `numpy` 2.5.2 (cp314 wheels, sha256-pinned) | — | MIT / BSD-3-Clause | files.pythonhosted.org | **no** — unzipped into the same `mktemp -d`, never `pip install`ed, deleted |
| **The rendered clips** | build output | Apache-2.0 (kidnix's own strings, this model) | <https://github.com/mattcree/kidnix> | **yes**, `/usr/share/kidnix/speech/en_GB/*.ogg` |

**Two credits are owed and are carried.** Kokoro v1.0's model card names two
CC-BY corpora in its training set. They cost two paragraphs, written out at
`/usr/share/kidnix/speech/ATTRIBUTION` and asserted line by line in
...
**A recorded caveat, which is not a licence question.** The Kokoro-82M model
card states that v1.0's training data includes *"synthetic audio generated by
closed TTS models from large providers"*, footnoted to US Copyright Office AI
policy guidance. Circumstantially this is what it looks like: five of Kokoro's
American voice names are the names of OpenAI's original six TTS voices. Nothing
about that changes the Apache-2.0 grant, and the copyright argument (machine
output is not itself copyrightable) is the one the card is gesturing at. The
**contract** argument is separate and unresolved — large TTS providers
generally forbid using their outputs to train competing models — and that is a
claim against hexgrad rather than against us. It is written down here, and in
the shipped `ATTRIBUTION`, rather than left as a silence. The Piper voices in
§6 have no equivalent question, and they remain the voice for every string that
is not in the catalogue.

**No GPL binary is redistributed, and none is linked.** The obvious route —
`kokoro-onnx` — depends on `phonemizer` (**GPL-3.0-or-later**) and
`espeakng-loader` (**no licence metadata at all**, and it ships a prebuilt
`libespeak-ng.so` plus 18 MB of `espeak-ng-data`: exactly the trap §6 already
made `65-tts.sh` delete out of the Piper tarball). Neither is installed.
Phonemes come from **Fedora's own** `espeak-ng(1)`, invoked as a subprocess at
build time, which `docs/spikes/tts-kokoro.md` §4.2 verified produces phonemes
identical to `phonemizer`'s on eleven real shell strings (11/11). So the whole
stage is Apache-2.0 + MIT + BSD.

**Why two manifest rows and not 314.** The clips are build *output* — kidnix
generated them from its own strings, the way `kidnix_shell/sound.py` generates
the five earcons — so what is third-party is the model, which is one fact about
the tree rather than 314 identical ones. `THIRD-PARTY.tsv` records the
`ATTRIBUTION` that covers the directory and the `index.json`, the same shape as
the GCompris letter-name clips' own `ATTRIBUTION`. `/usr/share/kidnix/speech`
is deliberately **not** in `test_licenses.sh`'s `VENDORED_TREES`;
