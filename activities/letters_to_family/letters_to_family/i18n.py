...
runs :mod:`letters_to_family.words`, :mod:`~letters_to_family.letter` and
:mod:`~letters_to_family.mailbox` on a machine that may have neither
``kidnix_activity`` on its path nor ``gi`` anywhere. A hard
``from kidnix_activity.i18n import _`` in :mod:`letters_to_family.words` would
