...
runs :mod:`clock_time.words`, :mod:`~clock_time.minute` and
:mod:`~clock_time.routine` on a machine that may have neither
``kidnix_activity`` on its path nor ``gi`` anywhere. A hard
``from kidnix_activity.i18n import _`` in :mod:`clock_time.words` would make
the words half of the activity depend on the window half, which is the exact
